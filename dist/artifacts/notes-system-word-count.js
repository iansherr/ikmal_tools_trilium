...
(() => {
  ...
  (function initLiveWordCount() {
    ...
    function updateEditorFooter(editor) {
      ...
      const summary = `ikmal editor \xB7 ${words} words (${goalPct}% of ${goalTarget}) \xB7 ${chars} chars \xB7 ${readingTimeMin} min read \xB7 Press ? for shortcuts`;
      const stateKey = `${summary} \xB7 ${issueSummary}`;
    ...
    function editorIssues(text) {
      ...
      const longSentences = text.split(/[.!?]+/).map((sentence) => countWords(sentence)).filter((sentenceWords) => sentenceWords > 40).length;
    ...
    function showSelectionMenu(text, paragraphText, noteWindow) {
      ...
      const stats = makeMenuElement(
        ...
        `${countWords(text)} words \xB7 ${chars} chars \xB7 ${charsNoSpaces} without spaces`
      );
