 ...
 * the launcher buttons use can be exercised by the test suite:
 *
...
/*
 * A ~template relation makes an instance inherit the template's attributes,
 * #extTemplate included, so getNoteWithLabel would start matching real notes as
 * soon as any exist. Ownership is the only reliable discriminator, so scan the
 * Templates container's children and check the attribute each one owns.
 */
...
function ensureHubDashboard(hub) {
    // Scripts live under _userHidden, which is intentionally excluded from
    // broad label searches. Walk that small implementation subtree instead.
...
function syncHubFromRound(hub, round = latestRoundForHub(hub)) {
    ...
    // Keep the Hub, current round, and Reporting Notes as one metadata view.
    // The current round wins when it has a value; otherwise a value entered
    // on the Hub or Reporting Notes flows to all of them. This makes the
    // common one-field edits bidirectional without inventing duplicate notes.
...
function findStorageParent(spec, hub) {
    ...
    if (spec.projectScoped) {
        // A quick capture without a project is still a real working note. Keep
        // it in the visible Projects tree so it can be assigned later.
...
function createScratch(title, projectId) {
    ...
    // Scratch notes deliberately have no ~template relation. They still use
    // the same project placement and day-note clone behavior as a story.
...
function createNote(typeKey, title, options = {}) {
    ...
    // Hubs are dateless by design -- cloning one into a day note would imply it
    // belongs to that day, which is exactly the confusion the hub avoids.
    let dayNoteId = null;
...
/*
 * #customRequestHandler endpoints are NOT authenticated by Trilium -- an
 * unguarded handler is an open note-creation endpoint for anyone who can reach
 * the server. Require a shared secret, stored as a label on this note and set
 * by apply_scripts.py, and fail closed if it was never configured.
 *
 * The secret is readable by anyone who can open this note in the UI, so it
 * guards against outside access, not against a user of the instance itself.
 * A multi-user deployment should move the buttons to api.runOnBackend and drop
 * the HTTP surface entirely.
 */
function isAuthorized(req) {
    // The secret lives on the #extConfig note rather than on this one, because
    // this note is packaged for distribution and that one is not.
